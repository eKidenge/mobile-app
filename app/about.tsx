...
function ProtectedLayout() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      // Add checks for algorithm-match and expert
      const isAlgorithmMatch = fullRoute === 'algorithm-match';
      const isExpert = fullRoute === 'expert';
      
      // If it's algorithm-match or expert route, ensure user is client
      if ((isAlgorithmMatch || isExpert) && user.user_type !== 'client') {
        console.log(`🚫 ${user.user_type} trying to access ${fullRoute}, redirecting`);
        if (user.user_type === 'professional') {
          router.replace('/professional/professional-dashboard');
        } else {
          router.replace('/dashboard');
  ...
  return (
    ...
    >
      ...
      {/* NEWLY ADDED ROUTES */}
      <Stack.Screen name="algorithm-match" />
